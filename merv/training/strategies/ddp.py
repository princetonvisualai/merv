...
class DDPStrategy(TrainingStrategy):
    ...
    def save_checkpoint(
        ...
        assert isinstance(self.vidlm, DDP), "save_checkpoint assumes VidLM is already wrapped in DDP!"

        # Splinter State Dictionary by Top-Level Submodules (or subset, if `only_trainable`)
        model_state_dicts = {
            mkey: getattr(self.vidlm.module, mkey).state_dict()
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        if self.enable_gradient_checkpointing:
            ...
            self.vidlm.llm_backbone.gradient_checkpointing_enable()

        # Move to Device =>> Note parameters are in full precision (*mixed precision* will only autocast as appropriate)
        overwatch.info("Placing Entire VidLM (Vision Backbone, LLM Backbone, Projector Weights) on GPU", ctx_level=1)
        self.vidlm.to(self.device_id)

        # Wrap with Distributed Data Parallel
        #   => Note: By default, wrapping naively with DDP(self.vidlm) will initialize a *separate* buffer on GPU that
        ...
        overwatch.info("Wrapping VidLM with Distributed Data Parallel", ctx_level=1)
        self.vidlm = DDP(self.vidlm, device_ids=[self.device_id], gradient_as_bucket_view=True)

        # Create Optimizer and LR Scheduler =>> note that most of the LR Schedulers we use require `max_steps/epochs`
        #   => Optimizer should only operate on parameters that are *unfrozen* / trainable!
        trainable_params = [param for param in self.vidlm.parameters() if param.requires_grad]
    ...
    def clip_grad_norm(self) -> None:
        torch.nn.utils.clip_grad_norm_(self.vidlm.parameters(), max_norm=self.max_grad_norm)
