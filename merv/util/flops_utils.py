...
from calflops.utils import flops_to_string, generate_transformer_input, macs_to_string, params_to_string
...
def get_statistics(
    ...
    """Calculate flops w/ an example input of [1 x max_len]."""
    llm_max_length = vidlm.llm_backbone.llm_max_length
    tokenizer = vidlm.llm_backbone.tokenizer
    dtype = vidlm.llm_backbone.half_precision_dtype

    char_choices = string.ascii_letters + string.digits + string.punctuation + " "
    input_ids = tokenizer(
        "".join(random.choices(char_choices, k=llm_max_length // 2)), truncation=True, return_tensors="pt"
    ).input_ids
    video_values = [torch.randn(frame, 3, 224, 224) for frame in num_frames]
    video_values = [
    ...
    flops, macs, _ = calculate_flops(model=vidlm, kwargs=dummy_input, print_results=False)
    params = sum(p.numel() for p in vidlm.parameters())
    return macs, params
...
def calculate_flops(
    ...
    if input_shape is not None:
        assert len(args) == 0 and len(kwargs) == 0, (
            "args and kwargs must be empty value if input_shape is not None, then will be generate random input by"
            " inpust_shape"
        )
        ...
        if transformer_tokenizer is None:  # model is not transformers model
            ...
            assert len(input_shape) == 2, (
                "the format of input_shape must be (batch_size, seq_len) if model is transformers model and"
                " auto_generate_transformers_input if True"
            )
            kwargs = generate_transformer_input(
                input_shape=input_shape, model_tokenizer=transformer_tokenizer, device=device
            )
    else:
        ...
        if transformer_tokenizer:
            kwargs = generate_transformer_input(input_shape=None, model_tokenizer=transformer_tokenizer, device=device)
